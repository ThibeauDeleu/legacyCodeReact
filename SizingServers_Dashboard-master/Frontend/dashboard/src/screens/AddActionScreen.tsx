...
export default function AddActionScreen() {
	...
	const splitUrl = () => {
		const type = location.pathname.substring(2).split('/')[2];
		setDataMovement(type);
	};
	...
	const [message, setMessage] = useState<string>('Could not create movement');
	...
	const [dataMovement, setDataMovement] = useState<string>('0');
			...
			<article className='c-form'>
				...
				<header className='c-form-header'>
					<div className='c-form-header-title__container'>
						...
						<h1 className='c-form-header-title'>Add movement</h1>
					</div>
					<button
